package convesorLinhaDigitavel;
import java.util.Scanner;
public class conversor {
	
		public static void main(String[] args) {
						
			System.out.println("Digite a linha digitável ou o código de barras");
			Scanner scanner = new Scanner(System.in);
			String input = scanner.nextLine();
			String linha = input.replaceAll("[^0-9]", "");
			if (linha.length() == 44) {
				System.out.println(calculaLinha(linha));
			}else if (linha.length() == 47) {	
				System.out.println(calculaCodBarra(linha));				
			}else {
		...
		public static String calculaLinha(String linha) {
			    ...
			    return  "Linha digitável: " + campo1 + modulo10(campo1)
			            +' '
